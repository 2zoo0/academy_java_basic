...
//	}
	...
	/**
	 * 매개변수로 넘겨진 제품 정보와 일치하는 제품 코드를 가잔
	 * 제품에 대해 입력된 값으로 수성하여 저장함
	 * @param product
	 */
	...
	private int findProductIdx(Product product) {
		...
		for (int idx = 0; idx < products.length; idx++) {
			...
			// product[idx] 의 제품코드 필드가 매개변수로 넘어온 product의 제품코드 필드와
			// 동일한지 비교하고 그때의 배열 인덱스를 저장
			if (products[idx].getProdCode().equals(product.getProdCode())) {
				index = idx;
				break;
			}
		}
